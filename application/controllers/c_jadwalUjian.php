<?php
class c_jadwalUjian extends CI_Controller
{
	...
	function index($id)
	{
        ...
        $jadwal = $this->m_jadwalGuru->jadwalUjian($id)->result();
		...
		$pdf->Cell(50, 1, '', 0, 1, 'C');
		$pdf->Cell(19, 0, 'JADWAL UJIAN SISWA SMAS ISLAMIC CENTRE', 0, 1, 'C');
		...
		$pdf->Cell(7, 0.7, 'Mata Pelajaran', 1, 1, 'C', 0);
		...
		foreach ($jadwal as $list) {
            if ($list->tanggal != $tglAkhir) {
                if ($list->hari == "Senin") {
                    ...
                    $pdf->Cell(7, 0.7, $list->namaMapel, 1, 1, 'L', 0);
                    $flagSenin = false;
                }
            }
		}
